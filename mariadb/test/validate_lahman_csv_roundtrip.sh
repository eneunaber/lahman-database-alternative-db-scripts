...
WORK_DIR="/tmp/lahman_mariadb_compare"

mkdir -p "$WORK_DIR"

read -rsp "MariaDB password for ${DB_USER}: " DB_PASSWORD
echo
...
for src in "$DATA_DIR"/*.csv; do
  ...
  table_exists=$(mariadb -h "$DB_HOST" -P "$DB_PORT" -u "$DB_USER" -p"$DB_PASSWORD" -N -s "$DB_NAME" -e \
...
if [[ "$overall_status" -eq 0 ]]; then
  echo "All CSV data matches after MariaDB export/normalize/sort comparison."
else
