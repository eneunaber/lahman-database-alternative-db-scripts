-- MariaDB conversion of create_lahman_mssql_script.sql
-- Run with the MariaDB client (example):
--   mariadb -h <server-name> -P 3306 -u <username> -p < create_lahman_mariadb.sql

CREATE DATABASE IF NOT EXISTS lahman CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE lahman;

DROP FUNCTION IF EXISTS TRANSLATE;
DELIMITER $$
CREATE FUNCTION TRANSLATE(input_text TEXT, from_chars TEXT, to_chars TEXT)
RETURNS TEXT
DETERMINISTIC
BEGIN
  DECLARE i INT DEFAULT 1;
  DECLARE from_len INT;
  DECLARE src_ch CHAR(1);
  DECLARE dst_ch CHAR(1);
  DECLARE out_text TEXT;

  SET out_text = input_text;
  SET from_len = CHAR_LENGTH(from_chars);

  WHILE i <= from_len DO
    SET src_ch = SUBSTRING(from_chars, i, 1);
    SET dst_ch = SUBSTRING(to_chars, i, 1);
    SET out_text = REPLACE(out_text, src_ch, dst_ch);
    SET i = i + 1;
  END WHILE;
...
SELECT playerID, awardID, yearID, lgID, CAST(notes AS CHAR) AS notes, COUNT(*) AS dup_count
FROM AwardsManagers
GROUP BY playerID, awardID, yearID, lgID, CAST(notes AS CHAR)
HAVING COUNT(*) > 1;
;
